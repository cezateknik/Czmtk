from PyQt5.QtWidgets import (QMainWindow, QWidget, QVBoxLayout, QHBoxLayout, 
                            ...
                            QCheckBox)
from PyQt5.QtCore import Qt, QThread, pyqtSignal, QRegExp, QSettings, QTimer, QDateTime
...
def normalize_version_string(version):
    try:
        return ".".join(str(int(part)) for part in str(version).split("."))
...
class UpdateCheckThread(QThread):
        ...
        try:
            ...
            if response.status_code == 200:
                ...
                if matches:
                    ...
                    latest_version = normalize_version_string(matches[-1].strip())
    ...
    def compare_versions(self, version1, version2):
        ...
        try:
            v1_parts = [int(x) for x in normalize_version_string(version1).split('.')]
            v2_parts = [int(x) for x in normalize_version_string(version2).split('.')]
...
class StartupUpdateDialog(QDialog):
    def __init__(self, latest_version, parent=None):
        ...
        self.setWindowTitle("Yeni Sürüm")
        self.setMinimumSize(420, 220)
        self.setWindowFlags(self.windowFlags() & ~Qt.WindowContextHelpButtonHint)
        self.setupUI()

    def setupUI(self):
        self.setStyleSheet("QDialog { background-color: #f5f7fb; }")
        ...
        badge_label = QLabel("GÜNCELLEME")
        badge_label.setAlignment(Qt.AlignCenter)
        badge_label.setStyleSheet(
            "QLabel { background-color: #eef3ff; color: #3859a6; border: 1px solid #d4def5; "
            "border-radius: 10px; padding: 4px 10px; font-size: 11px; font-weight: 700; }"
        )
        badge_label.setFixedWidth(104)
        card_layout.addWidget(badge_label, 0, Qt.AlignLeft)

        title_label = QLabel("CezaMat'ik için yeni sürüm mevcut")
        title_font = QFont()
        title_font.setPointSize(15)
        title_font.setBold(True)
        title_label.setFont(title_font)
        title_label.setStyleSheet("color: #1f2d46;")
        card_layout.addWidget(title_label)

        version_label = QLabel(f"Sürüm: {self.latest_version}")
        version_label.setStyleSheet("color: #3F51B5; font-size: 18px; font-weight: 700;")
        card_layout.addWidget(version_label)

        info_label = QLabel(
            "İsterseniz güncelleme sayfasını şimdi açabilirsiniz. "
            "Bu sürümü daha sonra hatırlatmak istemiyorsanız aşağıdaki kutuyu işaretleyebilirsiniz."
        )
        info_label.setWordWrap(True)
        info_label.setStyleSheet("color: #5a6578; font-size: 13px; line-height: 1.35;")
        card_layout.addWidget(info_label)

        divider = QFrame()
        divider.setFixedHeight(1)
        divider.setStyleSheet("background-color: #e8edf5; border: none;")
        ...
        self.ignore_checkbox = QCheckBox("Bir daha gösterme")
        self.ignore_checkbox.setStyleSheet("""
            QCheckBox {
                color: #2d3a4f;
                spacing: 8px;
                padding-top: 2px;
        ...
        """)
        button_row.addWidget(self.ignore_checkbox, 0, Qt.AlignVCenter)

        button_row.addStretch()

        later_button = QPushButton("Daha Sonra")
        later_button.setFixedSize(110, 38)
        ...
        update_button.setFixedSize(118, 38)
    ...
    def shouldIgnoreThisVersion(self):
...
class UpdateDialog(QDialog):
    ...
    def check_for_updates(self):
        ...
        self.update_thread = UpdateCheckThread(timeout_seconds=10)
...
<div style="margin-bottom: 20px;">
<h4 style="color: #2196F3; margin-bottom: 5px;">🔹 Sürüm 1.3.4 - 3 Nisan 2026</h4>
<ul style="margin-left: 20px; margin-bottom: 10px;">
<li>Dolandırıcılık modülü ile hesap makinesi arasına "Diğer Hesaplama Araçları" penceresi eklendi.</li>
<li>Hapsi Güne Çevirme, APC Orantılama ve Yaş Hesaplama araçları modüler bir yapıda uygulamaya dahil edildi.</li>
<li>Oran seçeneklerine yeni "1/3" tuşu eklendi.</li>
<li>Yeni eklenen araçların veri giriş alanları, buton hizaları, sonuç satırları ve başlık tasarımları yeniden düzenlendi.</li>
<li>Fare tekerleği ile sayısal ve tarih alanlarının istemsiz değişmesi engellendi.</li>
<li>Dolandırıcılık bölümündeki kopyalama butonu ve genel sıfırlama akışı iyileştirildi.</li>
<li>Hesap makinesi, adli para cezaları ondalık sisteme dönüştürüldü.</li>
</ul>
</div>
...
class MainWindow(QMainWindow):
    def __init__(self):
        ...
        self.history = []
        self.settings = QSettings("CezaMatik", "CezaMatik")
        self.startup_update_thread = None
        ...
        self.setupUI()
        self.connectSignals()
        QTimer.singleShot(0, self.checkForUpdatesOnStartup)
    ...
    def setupMenuBar(self):
        ...
        self.licenseAction = QAction("Lisans", self)
        ...
        self.licenseAction.triggered.connect(self.showLicenseDialog)
    ...
    def shouldCheckUpdatesOnStartup(self):
        return self.settings.value("update/check_on_startup", True, type=bool)

    def shouldSkipStartupCheckByTime(self):
        last_check = self.settings.value("update/last_check_at", "", type=str)
        if not last_check:
            return False

        last_check_dt = QDateTime.fromString(last_check, Qt.ISODate)
        if not last_check_dt.isValid():
            return False

        return last_check_dt.daysTo(QDateTime.currentDateTime()) < 30

    def checkForUpdatesOnStartup(self):
        if not self.shouldCheckUpdatesOnStartup():
            return

        if self.shouldSkipStartupCheckByTime():
            return

        self.settings.setValue("update/last_check_at", QDateTime.currentDateTime().toString(Qt.ISODate))
        self.startup_update_thread = UpdateCheckThread(timeout_seconds=3, silent_errors=True, parent=self)
        self.startup_update_thread.updateAvailable.connect(self.onStartupUpdateAvailable)
        self.startup_update_thread.finished.connect(self.onStartupUpdateFinished)
        self.startup_update_thread.start()

    def onStartupUpdateAvailable(self, latest_version):
        latest_version = normalize_version_string(latest_version)
        ignored_version = normalize_version_string(
            self.settings.value("update/ignored_version", "", type=str)
        )
        if ignored_version == latest_version:
            return

        dialog = StartupUpdateDialog(latest_version, self)
        result = dialog.exec_()

        if dialog.shouldIgnoreThisVersion():
            self.settings.setValue("update/ignored_version", latest_version)
        else:
            self.settings.setValue("update/ignored_version", "")
    ...
    def onStartupUpdateFinished(self):
        self.startup_update_thread = None
    ...
    def setupUI(self):
        ...
        copyright_label = QLabel("© İshak KOCATEPE (273098) | Haziran 2025")
        ...
        if not self.has_initial_values:
            ...
            if hasattr(self, 'initial_para') and self.initial_para and self.initial_para > 0:
                ...
                if text_parts:
                    ...
                    text_parts.append(f"{formatted_initial_para} Gün Adli Para Cezası")
